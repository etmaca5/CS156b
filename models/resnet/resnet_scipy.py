...
class SobelEdgeDetection:
    def __call__(self, img):
        img_np = np.array(img)
        gray = img_np.mean(axis=2)  # Convert to grayscale by averaging RGB channels

        # Apply the Sobel filter along x and y directions
        sobelx = sobel(gray, axis=0, mode='constant')
        sobely = sobel(gray, axis=1, mode='constant')

        # Compute the magnitude of the gradient
        sobel = np.sqrt(sobelx**2 + sobely**2)

        # Convert the gradient magnitude to a suitable format for an image
